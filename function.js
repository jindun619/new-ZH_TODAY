...
function dateIntoYMD(date) {
  const Year = date.getFullYear(),
    ...
    Date = date.getDate(),
    YMD = `${Year}-${Month < 10 ? `0${Month}` : Month}-${
      Date < 10 ? `0${Date}` : Date
    }`;
  return YMD;
}
...
function menuIntoBldObj(menuObj, paraBld) {
  let bldObj = menuObj.filter(function(value) {
    //this bld(breakfast/lunch/dinner)'s object
...
function paintMeal(menuObj, bld, lang) {
  //bld: breakfast=0;lunch=1;dinner=2
